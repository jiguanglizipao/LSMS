...
def do_accept_borrow(request):
	try:
		...
		message.append({'direction': 'Send', 'info_type': '', 'user_name': request.user.username, 'text': note})

		packed_update_borrow(request, id, {'status': ACCEPTED_KEY, 'note': message.tostring()},
		                     log=get_accept_brw_log())
...
def do_reject_borrow(request):
	try:
		...
		packed_update_borrow(request, id, {'status': REJECTED_KEY, 'note': message.tostring()},
...
def do_accept_return(request):
	try:
		...
		if (lost == 'false'):
			packed_update_borrow(request, id, {'status': RETURN_PENDING_KEY, 'note': message.tostring()},
			                     log=get_accept_ret_log())
		else:
			packed_update_borrow(request, id, {'status': LOST_KEY, 'note': message.tostring()}, log=get_lost_ret_log())
...
def do_accept_repair(request):
	try:
		...
		packed_update_borrow(request, id, {'status': REPAIR_PEND_KEY, 'note': message.tostring()},
...
def do_accept_destroy(request):
	try:
		...
		packed_update_borrow(request, id, {'status': DESTROY_ACCEPT_KEY, 'note': message.tostring()},
...
def do_reject_repair(request):
	try:
		...
		packed_update_borrow(request, id, {'status': BORROWED_KEY, 'note': message.tostring()},
...
def do_start_repair(request):
	try:
		...
		packed_update_borrow(request, id, {'status': REPAIRING_KEY, 'note': message.tostring()},
...
def do_finish_repair(request):
	try:
		...
		packed_update_borrow(request, id, {'status': FINISH_REPAIR_KEY, 'note': message.tostring()},
...
def do_borrow(request):
	try:
		...
		message = Message()
		message.append({'direction': 'Recv', 'info_type': '', 'user_name': request.user.username, 'text': note})
		brw = packed_create_borrow(request, sn=sgl.sn, status=BORROW_AUTHING_KEY, note=message.tostring(),
		                           account=account,
...
def do_return_goods(request):
	try:
		...
		packed_update_borrow(request, id, {'status': RETURN_AUTHING_KEY, 'note': message.tostring()},
...
def do_destroy_goods(request):
	try:
		...
		packed_update_borrow(request, id, {'status': DESTROY_APPLY_KEY, 'note': message.tostring()},
...
def do_repair_goods(request):
	try:
		...
		packed_update_borrow(request, id, {'status': REPAIR_APPLY_KEY, 'note': message.tostring()},
...
def show_borrow(request):
	...
	if 'filtdata' in request.POST:
		if request.POST['filtdata'] == 'true' or request.POST['filtdata'] == 'True' or request.POST[
			'filtdata'] == 'TRUE':
			print("true!")
			cont['filtdata'] = True

...
class MessageCenterTestCase(TestCase):
    ...
    def setUp(self):
        ...
        create_single(Goods.objects.get(id=1), ['g1', 'g3'], 'av', 'jiguanglizipao')
        create_single(Goods.objects.get(id=2), ['g2'], 'av', 'jiguanglizipao')
        create_single(Goods.objects.get(id=3), ['g5', 'g4'], 'un', 'jiguanglizipao')

        account = {'user_name': 'jiguanglizipao', 'password': 'jiguanglizipao',
                    'email': 'jiguanglizipao@jiguanglizipao.com',
                    'real_name': 'jiguanglizipao', 'department': ['a lab', 'b lab'],
                    'tel': '12345678',
        ...
        computing1 = {'pc_type': 'v', 'cpu': 'core', 'memory': 1024,
                      'disk': 100, 'disk_type': 's', 'os': 'windows',
                      'sn': 'c1',
                      'expire_time': date(year=2015, month=11, day=11) + timedelta(days=4),
                      'login': 'jiguanglizipao', 'password': 'jiguanglizipao',
        ...
        computing3 = {'pc_type': 'v', 'cpu': 'core', 'memory': 1024,
                      'disk': 100, 'disk_type': 's', 'os': 'windows',
                      'sn': 'c3',
                      'expire_time': date(year=2015, month=11, day=11) + timedelta(days=4),
                      'login': 'jiguanglizipao', 'password': 'jiguanglizipao',
        ...
        apply = Apply_Goods(name='test', sn='test', type_name='test1', account=account[0],
                            pro_names='Gender!!$$@#@#price!!$$@#@#', pro_values='Gender!!$$@#@#price!!$$@#@#')
        apply.save()
    ...
    def test_index(self):
        self.manager = Client()
        self.assertTrue(
            self.manager.login(username='jiguanglizipao', password='jiguanglizipao'))
    ...
    def test_import_database(self):
        ...
        file.write(string)
        file.close()
        wgname = self.manager.get(reverse('excel.views.import_database'), {'ran': 'test'})
        self.assertEqual(wgname.status_code, 200)
    ...
    def test_import_excel(self):
        ...
        file.close()
        wgname = self.manager.get(reverse('excel.views.import_excel'), {'ran': 'test'})
        self.assertEqual(wgname.status_code, 200)
        file = open('excel/test.xlsx', 'wb')
        file.write(export.content)
        file.close()
    ...
    def test_import_goods(self):
        ...
        for item in wgname.context['goods_list']:
            ...
            confirm = self.manager.post(reverse('excel.views.import_goods'), item)
    ...
    def test_import_computing(self):
        ...
        for item in wgname.context['computing_list']:
            confirm = self.manager.post(reverse('excel.views.import_computing'), item)
            self.assertEqual(confirm.status_code, 200)
            self.assertEqual(confirm.content.decode(), 'Success')
    ...
    def test_exception(self):
        ...
        self.assertIn('Error', exc.content.decode().split(' '))
        self.assertRaises((Exception, self.manager.get, reverse('excel.views.import_excel')))
        self.assertRaises((Exception, self.manager.get, reverse('excel.views.import_database')))

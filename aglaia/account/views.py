...
def do_auth_email(request, username, ekey):
    ...
    try:
        ...
        if account.email_hash == ekey:
            ...
            account.save()
            return show_message(
                request, "Your Email has been authenticated! Welcome to Aglaia!")
    except:
        pass
        ...
        if request.user.is_authenticated():
            ...
            if not user or not user.is_active:
                ...
                if not packed_find_users(request, {'user_name': username}, {}):
                    ...
                    account.save()
                    check_type(account)
                if packed_find_users(request, {'user_name': username}, {})[
                        0].type == 'none':
...
def do_modify_account(request):
    ...
    try:
        rname = post['realname']
        schid = post['number']
        if (rname or schid) and request.user.status != status_unauth_key and not request.user.has_perm(
                PERM_MODF_KEY):
...
def do_modify_password(request):
    ...
    try:
        ...
        if user.check_password(old_pass):
            packed_update_user(request, Account.objects.get(user=request.user).id, {
                               'password': new_pass}, log=get_pass_modif_log())
            return HttpResponse('ok')
        else:
            return HttpResponse('mismatch')
    except Exception as e:
...
def do_modify_email(request):
    ...
    packed_update_user(
        request, account.id, {
            'email': email}, log=get_email_modif_log())
...
def do_approve_account(request):
    try:
        ...
        if not account or not account.user.is_active or account.status == status_destroy_key:
            return HttpResponse('denied')
        packed_update_user(request,
                           account.id,
                           {'status': status_authed_key},
                           log=get_approve_account_log())
...
def do_disapprove_account(request):
    try:
        ...
        packed_update_user(request,
                           account.id,
                           {'status': status_unauth_key},
                           log=get_disapprove_account_log())
        return HttpResponse('ok')
    except:
        return HttpResponse('denied')
...
def do_delete_account(request):
    try:
        ...
        packed_update_user(request,
                           account.id,
                           {'status': status_destroy_key},
                           log=get_delete_account_log())
        return HttpResponse('ok')
    except:
        return HttpResponse('denied')
